...
export class NgDatepickerComponent
  ...
  setOptions(): void {
    ...
    this.displayFormat =
      (this.options && this.options.displayFormat) || 'MMM D[,] YYYY';
    this.barTitleFormat =
      (this.options && this.options.barTitleFormat) || 'MMMM YYYY';
    this.dayNamesFormat =
      (this.options && this.options.dayNamesFormat) || 'ddd';
    this.barTitleIfEmpty =
      (this.options && this.options.barTitleIfEmpty) ||
    ...
    this.fieldId =
      (this.options && this.options.fieldId) || this.defaultFieldId;
    this.useEmptyBarTitle =
      this.options && 'useEmptyBarTitle' in this.options
        ? this.options.useEmptyBarTitle
        : true;
  }
  ...
  init(): void {
    ...
    if (this.innerValue) {
      ...
      this.barTitle = this.useEmptyBarTitle
        ? this.barTitleIfEmpty
        : format(start, this.barTitleFormat, this.locale);
    }
  }
  ...
  onInputChange() {
    ...
    if ((/^([0][0-9]|[12][0-9]|3[01])\/([0][0-9]|(1)[0-2])\/\d{4}$/).test(this.displayValue)) {
      const [ day, month, year] = this.displayValue.split('/');
